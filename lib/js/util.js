  ...
  return new Promise(function(resolve, reject) {
    ...
    mysql_util
      ...
      .then(function(result_obj) {
        ...
        let body = null;
        let form_Data = null;
        if (request.body && (body = JSON.parse(request.body)).attachmentUrl) {
          req_body = objectToFormData(body);
          api_headers = req_body.getHeaders();
        } else if (request.x_file_content) {
...
/**
 * Returns the properly form encoded value for the `obj`
 * object provided.
 *
 * @param   {Object}   obj JSON object to convert to form data
 * @param   {FormData} formdata current form data object
 * @param   {string}   namespace namespace of the current properties
 * @returns {FormData} Object as form data
 */
